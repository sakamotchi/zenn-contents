---
title: "Claude Codeの「許可待ち」をOSのネイティブ通知で受け取る仕組みをSpecPromptに実装した"
emoji: "🔔"
type: "tech"
...
仕様駆動開発のための軽量デスクトップアプリ「**SpecPrompt**」に、**Claude Code通知機能** を実装しました。Claude Codeの「許可待ち」「処理完了」「エラー」などをOSのネイティブ通知として受け取れるようになります。

https://github.com/sakamotchi/spec-prompt

SpecPrompt自体の概要は以下の記事を参照してください。
...
SpecPromptは2つの経路でClaude Codeのイベントを検出します。
...
Claude Codeをはじめ、対応したCLIツールがこのシーケンスを出力すると、SpecPromptのターミナルエンジン（alacritty_terminal）がイベントを受信し、ネイティブ通知を発火します。
...
- デフォルト: 有効
- 設定値は `~/.config/spec-prompt/config.json` の `appearance.notification_enabled` に永続化
- アプリ再起動後も設定が引き継がれる
...
これでClaude Codeが許可待ち・完了・エラーなどのフックを発火するたびにSpecPromptに届きます。

# おわりに

「Claude Codeに任せて別のことをする」というスタイルが定着するほど、**処理の節目で気づける通知** の重要性が増してきました。OSC 9とHTTPフック、二経路で取りこぼしを減らし、タブ名差し込み・未読マーク連動まで含めて、SpecPromptの「軽量さを保ったまま実用度を上げる」方針に沿った機能になったと思います。
